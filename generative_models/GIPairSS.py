...
class GIPair(object):
    ...
    Parameters:
        ...
        t_net: The "top" net that will serve for classificatory porpoises
              -- this only needs to provide three things:
                 1. a list holding its "optimizable" parameters 
                 2. a cost function that depends on its input and a list of
                    integer-valued labels, where 0 indicates "no label"
                 3. a symbolic var that provides the "class predictions" for
                    the values currently linked through its symbolic input
        data_dim: dimension of the "observable data" variables
        prior_dim: dimension of the "latent prior" variables
    """
    def __init__(self, rng=None, g_net=None, i_net=None, t_net=None,
...
if __name__=="__main__":
    ...
    GIP = GIPairSS(rng=rng, g_net=GN, i_net=IN, data_dim=data_dim, \
