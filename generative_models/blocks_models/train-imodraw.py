...
def main(name, epochs, batch_size, learning_rate, attention,
    ...
    mix_enc_mlp = MLP([Tanh()], [x_dim, 500], name="MLP_mix_enc", **inits)
    mix_sampler = Qsampler(input_dim=500, output_dim=mix_dim, **inits)
    mix_dec_mlp = MLP([Tanh(), Tanh()], \
                      [mix_dim, 500, (2*enc_dim + 2*dec_dim + mix_dim)], \
                      name="MLP_mix_dec", **inits)
    # setup the components of the generative DRAW model
    encoder_mlp = MLP([Identity()], [(read_dim + dec_dim + mix_dim), 4*enc_dim], \
                        name="MLP_enc", **inits)
    decoder_mlp = MLP([Identity()], [             (z_dim + mix_dim), 4*dec_dim], \
                        name="MLP_dec", **inits)
    encoder_rnn = LSTM(dim=enc_dim, name="RNN_enc", **rnninits)
    decoder_rnn = LSTM(dim=dec_dim, name="RNN_dec", **rnninits)
    draw_sampler = Qsampler(input_dim=enc_dim, output_dim=z_dim, **inits)

    draw = IMoDrawModels(
                n_iter,
                mix_enc_mlp=mix_enc_mlp,
                mix_sampler=mix_sampler,
                mix_dec_mlp=mix_dec_mlp,
                reader=reader,
                encoder_mlp=encoder_mlp,
                encoder_rnn=encoder_rnn,
                draw_sampler=draw_sampler,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mix-dim", type=int, dest="mix_dim",
                default=32, help="Continuous mixture dimension")
